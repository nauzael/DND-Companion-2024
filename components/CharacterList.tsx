...
const CharacterList: React.FC<CharacterListProps> = ({ characters, onCreate, onSelect, onDelete, onExport, onImport }) => {
  ...
  return (
    <div className="flex flex-col h-full min-h-screen pb-24 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {characters.map((char) => (
                  <article 
                    ...
                    className="group relative flex items-center gap-4 bg-white dark:bg-surface-dark p-3 rounded-2xl shadow-sm border border-transparent hover:border-primary/30 transition-all duration-300 overflow-hidden cursor-pointer"
                  >
                    ...
                    <div className="relative shrink-0">
                      ...
                      ></div>
                      <div className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-slate-900 ring-2 ring-white dark:ring-surface-dark">
      ...
      <div className="fixed bottom-8 right-6 z-30 mb-[env(safe-area-inset-bottom)] md:right-12">
        <button 
            onClick={onCreate}
